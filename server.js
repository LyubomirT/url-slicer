...
function log(message, data = {}) {
  console.log(
    JSON.stringify({ timestamp: new Date().toISOString(), message, ...data })
  );
}
...
app.post("/shorten", async (req, res) => {
  ...
  if (customAlias && (customAlias.length < 3 || customAlias.length > 50)) {
    return res
      .status(400)
      .json({ error: "Custom alias must be between 3 and 50 symbols" });
  }
...
app.get("/api/analytics", async (req, res) => {
  ...
  try {
    const totalClicks = await Click.countDocuments({
      url_id: {
        $in: await Url.find({ user_id: req.user._id }).distinct("_id"),
      },
    });
  ...
  try {
    ...
    if (geo) {
      ...
      try {
        ...
        log("Failed to detect country from external API", {
...
app.post("/:code/verify", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while verifying the password" });
  }
});
...
async function getTotalClicks(userId) {
  return Click.countDocuments({
    url_id: { $in: await Url.find({ user_id: userId }).distinct("_id") },
  });
}
